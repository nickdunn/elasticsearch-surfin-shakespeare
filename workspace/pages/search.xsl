...
<xsl:template match="data/elasticsearch" mode="filters">
	...
	<ul class="filters">
		<li>
			...
			<a href="/search/?keywords={$keywords}">
				Everything
				<span>
		...
		<xsl:for-each select="facets/facet[@handle='all-sections']/term">
			<li>
				...
				<a href="/search/?keywords={$keywords}&amp;sections={@handle}">
...
<xsl:template match="data/elasticsearch" mode="results">
	...
	<h2>
		...
		<xsl:if test="$sections!=''">
			<span>
				...
				<xsl:for-each select="facets/facet[@handle='filtered-sections']/term[@active='yes']">
					<xsl:value-of select="text()"/>
	...
	<xsl:choose>
		<xsl:when test="entries/entry">
			<xsl:apply-templates select="entries/entry" mode="result"/>
		</xsl:when>
		<xsl:otherwise>
